...
public class ByteBitsDesc {
    ...
    public void setDesc(String desc) {
        ...
        if (!getByteIndex()) {
            ...
            Logcat.e("error BYTE format " + desc);
        } else {
            if (!getBitMask()) {
                mFormatIsValid = false;
                Logcat.e("error BIT format " + desc);
            } else {
                mFormatIsValid = true;
            }
        }
    ...
    public void set(byte[] buff, int value) {
        ...
        if (value > (mBitMask >> mBitLowIndex)) {
            Logcat.e("Warning value is overflow at " + mDesc + " value is " + value);
        }
    ...
    private boolean getBitMask() {
        ...
        if (start < 0) {
            Logcat.e("Can not find " + BIT_NAME + " from " + mDesc);
            return false;
        }

        start = mDesc.indexOf('[', start);
        if (start < 0) {
            Logcat.e("Can not find [ after BIT from " + mDesc);
            return false;
        }
    ...
    public static void test() {
        ...
        for (int i=0; i<TEST_COUNT; i++) {
            ...
            desc.set(buff, i);
            if (buff[i] != i) {
                Logcat.e("TEST set error at " + desc.mDesc);
            }
        }
        ...
        for (int i=0; i<TEST_COUNT; i++) {
            ByteBitsDesc desc = new ByteBitsDesc("Byte["+i+"]");
            if (desc.get(buff) != i) {
                Logcat.e("TEST get error at " + desc.mDesc);
            }
        }
        ...
        for (int i=0; i<TEST_COUNT; i++) {
            ...
            if (value != (i % 2)) {
                Logcat.e("TEST set error at " + desc.mDesc + ", got " + value);
            }
        }
